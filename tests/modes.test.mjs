...
import { MODES, strikeKind, isOpen } from '../src/combat.js';
import { STEP, TILE } from '../src/tuning.js';
import { intent } from './helpers.mjs';

/**
 * Режимы боя сравниваются руками, но их правила должны быть однозначны.
 * Тесты держат именно правила: что считается заходом со спины, что делает
 * парирование и чем однохитовая смерть отличается от дуэльной.
 */
...
test('спиной страж не видит — иначе зайти сзади невозможно в принципе', () => {
    const world = createWorld(ARENA, 'edge');
...
test('в дуэли снятия нет: со спины проходит обычный удар', () => {
    const world = createWorld(ARENA, 'duel');
    const foe = world.enemies[0];
    foe.facing = -1;
    const p = world.player;
    p.body.x = foe.body.x + 30;
    ...
    swing(world, 0.3);
    assert.equal(world.takedowns, 0);
    assert.equal(foe.hp, MODES.duel.enemy.hp - 1);
});

test('однохитовая смерть откатывает к чекпоинту, а не заканчивает игру', () => {
    const world = createWorld(undefined, 'edge');
    const p = world.player;
    const start = { ...world.checkpoint };

    p.hp = 0;
    stepWorld(world, intent(), STEP);
    assert.equal(world.phase, 'play', 'однохитовый режим объявил конец игры');
    ...
    assert.equal(p.hp, MODES.edge.player.hp);
    assert.equal(p.body.x, start.x);
    assert.ok(world.enemies.every((e) => e.state !== 'dead'), 'стражи не поднялись');
});

test('в дуэли смерть по-прежнему заканчивает забег', () => {
    const world = createWorld(undefined, 'duel');
    world.player.hp = 0;
    stepWorld(world, intent(), STEP);
    assert.equal(world.phase, 'lost');
});

test('чекпоинты берутся по пути и только вперёд', () => {
    const world = createWorld(undefined, 'mix');
