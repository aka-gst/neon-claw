...
function playerAttacks(world) {
    ...
    for (const e of world.enemies) {
        ...
        if (result === 'blocked') {
            ...
            if (result === 'dead') {
                // Снятие со спины — ОТДЕЛЬНАЯ механика с 7 сентября: удар
                // по не знающему о тебе стражу убивает разом, при любом
                // здоровье. Смотреть `hurtEnforcer`, ветка `fromBehind &&
                // !isAware`. Здесь счётчик только считает — убийство уже
                // случилось выше.
                //
                // До этого удар снимал обычные четыре из шести, страж
                // выживал и разворачивался, и счётчик не рос никогда.
