...
window.NEON = {
    ...
    /**
     * Стражи текущего мира. Заведено потому, что `play()` и `restart()`
     * ПЕРЕСОЗДАЮТ мир: ссылка `const foe = NEON.world.enemies[0]`, взятая
     * до них, остаётся указывать на мёртвую копию. Снаружи это выглядит
     * дико — шаги исполняются, а разрыв до стража не меняется вовсе.
     * Оператор потерял на этом два часа: держал старого стража, живой
     * уходил в патруле, герой застревал в 139 точках.
     *
     * Обращаться так: `NEON.enemies[0]` каждый раз, а не сохранять в
